...
CI runs the installer on Linux but never starts REAPER or Sforzando, so run this before a release that changes the Linux install path (`install.sh`, Setup Tracks, `dev/dev_reset.sh`). Start from a Linux machine with no previous MIDI-GPT or Sforzando install (run `./dev/dev_reset.sh` first if it has one): REAPER installed from its Linux tarball and launched once, plus Python 3.10–3.12 and git.
