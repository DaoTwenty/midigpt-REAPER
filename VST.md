...
**Linux:** Sforzando for Linux is a **beta** from Plogue — it works with MIDI-GPT's automatic setup (VST3, tested on Ubuntu 24.04), but may be less stable than the macOS/Windows builds. It comes as a zip on Plogue's [downloads page](https://www.plogue.com/downloads.html#sforzando) rather than a single installer: unzip it and run `./install_sforzando.sh` in a terminal (without `sudo`; it asks for your password itself). There's no format choice — it always installs both VST3 (`/usr/lib/vst3`) and CLAP (`/usr/lib/clap`), plus the Aria engine and the SoundFont converter Setup Tracks uses under `/opt/Plogue`. REAPER on Linux only scans `~/.vst` and `~/.vst3` by default, so `install.sh` adds `/usr/lib/vst3` to its VST paths; without that, only the unsupported CLAP build shows up. On a minimal system without a desktop environment, also install `libxcb-util1` (the engine needs it, but Plogue's packages don't declare it).
...
2. Inside Sforzando: **Instrument → Import**, select your `.sf2` file (on Linux this is the only way — dropping files onto Sforzando's window doesn't work there), then **Instrument → Converted → sf2 →** pick the GM program you want. Each program number corresponds to a GM instrument — this mapping is what generated MIDI expects for correct playback (see [INSTRUMENTS.md](INSTRUMENTS.md)).
